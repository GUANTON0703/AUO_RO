...
def _rename_learned_skills(conn: sqlite3.Connection, renames: dict[str, str]) -> None:
...
def _migration_3(conn: sqlite3.Connection) -> None:
    """技能改名（B4）：騎士 shield_charge → counter_attack。"""
    _rename_learned_skills(conn, {"shield_charge": "counter_attack"})


def _migration_4(conn: sqlite3.Connection) -> None:
    """技能改名（B5）：巫師 frost_armor → frost_nova。"""
