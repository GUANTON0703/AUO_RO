...
## 2026-09-07 第三批：掛機即時感（已 merge master，373 passed）

使用者實測回報「掛機下去要等 15 秒、時間數字都卡住、按 enter 回主畫面像停了」。

- 結算防抖 15s → 3s。真正防浪費改靠「線上結算只用掉湊完整場戰鬥的時間，
  剩下留給下次」（`SettlementResult.consumed_seconds`）。一直輪詢也不燒零碎時間。
- 沒湊出戰鬥的結算直接撤銷不寫入，時間繼續攢。
- 自動選怪門檻 0.6 → 0.85 + 「≤12 回合」+「掉血 ≤55% 血量」，固定種子穩定判斷。
  起始怪挑「打得穩之中升級最快」的（`base_exp / avg_rounds`）。
- 只在有擊殺時才輪替目標，攢時間不動目標。
- 可打怪清單快取（`_hunt_meta`），鍵涵蓋所有戰力來源。
- 主選單掛機時間本地補間，每次重繪看得到在動。
- 「按 enter 回主畫面停止掛機」→ 其實沒停，是數字卡住的錯覺，補間修掉。

### 使用者還想要（尚未做，屬新功能不是 bug）
- 掛機設定選單：指定自動買什麼水、自動賣哪些物品（`strategy_menu` 目前只有開關）
- 背包：勾選某物品之後掛機都自動賣
- 內容查詢：卡片能插在哪個部位（`content_menu` 目前只有怪物掉落 / 物品效果 / 裝備條件）
- 這些都用上下選單挑選的互動方式

## 2026-09-07 第二批：GM 工具、主選單常駐面板、畫面修復（已 merge master）
