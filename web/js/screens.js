...
Screens.home = {
  ...
  _potionHtml(status) {
    ...
    return `
      <details style="margin-top:4px">
        <summary style="cursor:pointer" class="dim">HP 藥水（本場已喝 <span id="hk-pot-hp-used">${c.hunt_potions_used ?? 0}</span> 瓶・點開選要喝哪瓶）</summary>
        ${group("pot-hp", "heal_hp", strat.potion_item_id)}
      </details>
      <details style="margin-top:4px">
        <summary style="cursor:pointer" class="dim">SP 藥水（本場已喝 <span id="hk-pot-sp-used">${c.hunt_sp_potions_used ?? 0}</span> 瓶・點開選要喝哪瓶）</summary>
  ...
  async _refreshEquip() {
    ...
    if (pb) {
      const openFlags = [...pb.querySelectorAll("details")].map((d) => d.open);
      pb.innerHTML = this._potionHtml(this._lastStatus);
      pb.querySelectorAll("details").forEach((d, i) => { if (openFlags[i]) d.open = true; });
      this._wirePotion();
    }
  },
